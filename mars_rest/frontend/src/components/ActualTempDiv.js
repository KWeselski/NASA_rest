...
export default class TemperatureDiv extends Component{      
    ...
    get_data(){
        ...
        this.state.date = String(da.first_utc).split('T');
        this.state.h_temp = String(da.max_temp).split('.')
        this.state.m_temp = String(da.min_temp).split('.')
        this.state.a_temp = String(da.avg_temp).split('.')
    ...
    render(){
        ...
        if(this.state.toggled == true){celsius = this.switchTemperature(true); degree=f}
        else{
